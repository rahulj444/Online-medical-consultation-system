...
def afterdoctorlogin(request):
    my_id = request.session.get('doctor_id')
    
    # Redirect if no valid session
    if not my_id or my_id == 'default_id':
        return redirect('/doctorlogin')
    ...
    response = render(request, 'after_doctor_login.html', {'my_doctor': my_doctor})
    response.set_cookie('doctor_name', d.name, max_age=3600)  # Cookie valid for 1 hour here
    return response
...
def book_appointment(request):
    ...
    if not patient_id or isinstance(patient_id, str):
        return redirect('/patientlogin')
    ...
    if request.method == 'POST':
        ...
        pincode = request.POST.get('pincode')
        specialization = request.POST.get('specialization')
        hospital_id = request.POST.get('hospital')  # Changed from hospital_name
        ...
        if hospital_id:
            queryset = queryset.filter(hospitalid=hospital_id)
            filters_applied = True

        if city:
            hospitals_in_city = hospital.objects.filter(address__icontains=city)
            queryset = queryset.filter(hospitalid__in=hospitals_in_city)
            filters_applied = True

        # Check if any doctors found
        if not queryset.exists():
            msg = "No doctors found matching your criteria"
            if pincode:
                msg += f" in pincode {pincode}"
            if specialization:
                spec_name = dict(doctor.SPECIALIZATION_CHOICES).get(int(specialization), '')
                msg += f" for {spec_name}"
            messages.warning(request, msg)
            return redirect('/afterpatientlogin/')

        # Prepare session data
        request.session['doctors_list'] = [{
            'id': doc.id,
            'name': doc.name,
            'specialization': doc.get_specialization_display(),
            'hospital_name': doc.hospitalid.name,
            'hospital_address': doc.hospitalid.address
        } for doc in queryset.select_related('hospitalid')]

        return redirect('/select_doctor')

    # GET request - show form with hospitals
    hospitals = hospital.objects.all()
...
def ask_doctor_date_time(request):
    if request.method == 'POST':
        ...
        # Collect all appointments for this doctor in the next 7 days
        week_appointments = bookedappointments.objects.filter(
            doctorid=doctor_id,
            date__range=[today, seven_days_from_now]
        )

        if not week_appointments.exists():
            messages.info(request, "No appointments scheduled for you in the upcoming week.")

        context = {
            'today': today.strftime("%Y-%m-%d"),
            'seven_days_from_now': seven_days_from_now.strftime("%Y-%m-%d"),
...
def show_pending_doctors(request):
    ...
    if request.method == 'POST':
        ...
        if not pending_doctors_list:
            messages.info(request, "No pending doctors available..!")
